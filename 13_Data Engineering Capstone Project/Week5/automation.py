...
# Connect to MySQL
# connect to database
connection = mysql.connector.connect(user='root', password='OTAxNC10amd1czQy',host='127.0.0.1',database='sales')
...
dsn_uid = "rdy73707"        # e.g. "abc12345"
dsn_pwd = "33yMjb8oC4c2mFji"      # e.g. "7dBZ3wWt9XN6$o0J"
...
def get_latest_records(rowid):
	...
	sql = f'''
	SELECT rowid, product_id, customer_id, 0 as price, quantity, '{timestamp}' as timestamp
	FROM sales_data
	WHERE rowid > {rowid}
	'''
...
def insert_records(records):
	for record in records:
		db2_sql = ibm_db.prepare(conn, f"INSERT INTO sales_data VALUES{record};")
		ibm_db.execute(db2_sql)
